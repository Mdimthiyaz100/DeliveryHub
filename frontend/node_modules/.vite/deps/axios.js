...
var toObjectSet = (arrayOrString, deer) => {
  const obj = {};
  const define = (arr) => {
    arr.forEach((value) => {
      obj[value] = true;
  ...
  isArray(arrayOrString) ? define(arrayOrString) : define(String(arrayOrString).split(deer));
  return obj;
};
var noop = () => {
};
...
function toFormData(obj, formData, options) {
  ...
  function stringifyWithDepth(value, depth) {
    ...
    const ancestors = [];
    return JSON.stringify(value, function Depth(_key, currentValue) {
  ...
  function defaultVisitor(value, key, path) {
    ...
    if (value && !path && typeof value === "object") {
      if (utils_default.endsWith(key, "{}")) {
        ...
        value = stringifyWithDepth(value, 1);
...
var factory = (env) => {
  ...
  return async (config) => {
    ...
    const maxBodyLengthError = () => new AxiosError_default(
      "Request body larger than maxBodyLength ",
      AxiosError_default.ERR_BAD_REQUEST,
      config,
      request
    );
